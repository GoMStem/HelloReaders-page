...
export async function generateMetadata({
  ...
  return lang === 'en'
    ? { title: 'Daily Posting | Hello Readers', description: 'Daily updates from the Hello Readers classroom.' }
    : { title: 'Daily Posting | Hello Readers', description: 'Hello Readers의 일상 이야기와 소식' };
}
...
export default async function DailyPage({
  ...
  const t = ui[lang];
  const posts = getAllDailyPosts(lang);
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {posts.map((post) => {
              ...
              return (
                <Link
                  key={post.slug}
                  href={`/daily/${post.slug}?lang=${lang}`}
